...
## PLAYANIM
> playanim \<캐릭터: CharacterType> \<애니메이션 클립 이름: ClipName> [재생 시간: float]
...
## ClipName
애니메이션 클립의 이름을 나타내는 문자열입니다. 만약 캐릭터에 Animator 컴포넌트가 존재하고, Animator가 `앞으로 걷기`라는 이름의 애니메이션 클립을 포함하고 있다면 다음과 같이 애니메이션을 재생할 수 있습니다.
```
playanim bona "앞으로 걷기"
```
그러나 애니메이션 클립 이름이 자주 사용되는데 항상 위와 같이 따옴표로 감싼 긴 이름을 사용하는 것은 상당히 불편할 수 있습니다. 이 경우 `AnimationPalette` 컴포넌트를 사용하여 애니메이션 클립 이름의 별칭(Alias)를 정의할 수 있습니다.
`AnimationPalette` 컴포넌트는 `Animator` 컴포넌트와 동일한 오브젝트에 부착되어야 합니다.

만약 `AnimatorPalette`에서 `앞으로 걷기` 애니메이션 클립 이름을 `WALK_F`로 정의하였다고 가정합니다. 이제 ClipName은 원래 클립 이름 뿐만 아니라 새로 정의된 별칭도 허용하게 됩니다. 따라서 `앞으로 걷기` 대신 `WALK_F`를 사용할 수 있습니다.
```
